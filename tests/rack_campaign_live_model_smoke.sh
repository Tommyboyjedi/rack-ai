...
find "$rack/state/campaigns/live-two-step-fallback/steps/fallback" -name worker-transcript.json -exec grep -l '"attempt_kind": "fallback"' {} + | grep -q .
if find "$rack/state/campaigns/live-two-step-fallback/steps" -name worker-transcript.json -exec grep -E '"used_host_shell": true|jcode' {} +; then
  echo "FAIL: worker transcript indicates forbidden host shell/JCode use" >&2
  exit 1
fi
find "$rack/state/campaigns/live-two-step-fallback/steps" -name worker-transcript.json -exec grep -q '"executor_kind": "podman"' {} \;
