...
class RuntimeTests(unittest.TestCase):
    ...
    def test_equal_concurrent_and_opposite_resource_order(self):
        ...
        self.assertEqual(sum(d['state']!='unavailable' for d in grants),1)
        winner = r.wait(next(d for d in grants if d['state']!='unavailable'))
    ...
    def test_policy_identity_and_denial_replay(self):
        ...
        denied = r.acquire('athba','local-primary','low',identity='unavailable')
        self.assertEqual(denied['state'],'unavailable')
        r.release(d); r.wait(d,'released')
        retried=r.wait(r.call('athba','acquire',request=denied['request']))
        self.assertNotEqual(retried['id'],denied['id'])
    ...
    def test_cancel_preempted_reservation_never_restores_or_dispatches(self):
        r = self.rack
        p = r.wait(r.acquire('athba','local-primary','low'))
        chat = r.wait(r.acquire('cb','local-fun-chat','paramount'))
        p = r.wait(p,'preempted')
        request=dict(schema=VERSION,submission_id='preempted-work',reservation_id=p['id'],generation=p['generation'],profile_hash=p['profile_hash'],prompt='blocked',max_tokens=16,timeout_seconds=5)
        r.call('athba','infer',status=409,request=request)
    ...
    def test_stale_generation_rejected_and_tag_rebinding_is_frozen(self):
        ...
        chat = r.wait(r.acquire('cb','local-fun-chat','paramount'))
        r.wait(p,'preempted');r.release(chat);r.wait(chat,'released')
        r.call('athba','infer',status=409,request=dict(request,submission_id='stale'))
        fresh=r.wait(r.acquire('athba','local-primary','low',identity='stale-explicit-reacquire'))
        self.assertNotEqual(fresh['generation'],p['generation'])
        self.assertEqual(fresh['profile_hash'],p['profile_hash'])
    ...
    def test_startup_model_mismatch_leaves_displaced_demand_preempted(self):
        r = self.rack
        p = r.wait(r.acquire('athba','local-primary','low'))
        ...
        replacement=r.wait(r.acquire('athba','local-primary','low',identity='mismatch-explicit-reacquire'))
        self.assertNotEqual(replacement['id'],p['id'])
    ...
    def test_opposite_resource_order_alias_and_frozen_profile_after_restart(self):
        ...
        self.assertEqual(sum(g['state']!='unavailable' for g in grants),1)
        winner=r.wait(next(g for g in grants if g['state']!='unavailable'))
    ...
    def test_cancel_takeover_during_drain_finishes_terminal_preemption(self):
        r=self.rack
        p=r.wait(r.acquire('athba','local-primary','low'))
        r.controls('local-primary',delay=1)
        invocation=r.infer(p); r.result(invocation,'running')
        chat=r.acquire('cb','local-fun-chat','paramount')
    ...
    def test_simulated_reboot_or_stale_pid_quarantines_ownership(self):
        ...
        self.assertEqual(denial['state'],'unavailable')
        self.assertEqual(r.counts('start')['local-fun-chat'],0)

    def test_preempted_demand_never_resurrects_after_deadline(self):
        r=self.rack
        ...
        r.release(chat);r.wait(chat,'released')
        self.assertEqual(r.counts('start')['local-primary'],1)
        self.assertEqual(r.inspect(p)['state'],'preempted')
