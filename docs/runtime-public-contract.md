...
are explicitly bounded. HTTP request bodies are limited to 1 MiB. Each accepted invocation freezes its response byte bound. The production/operator-approved `limits.max_response_bytes` baseline is **3 MiB** (`3145728` bytes).
...
The gateway enforces qualified protocols and output bounds. For existing JCode/review clients that omit an output bound, it supplies the frozen profile maximum before recording and dispatching the complete request; explicit bounds cannot exceed that maximum. Profiles that publish `image_input` accept bounded Chat Completions `image_url` parts only through the scoped gateway. Responses image input is not advertised or accepted. The gateway accepts only `data:image/...;base64,...` values; it does not fetch remote URLs or read server-side filesystem paths. CLI callers may pass local image paths, but those files are encoded client-side before the gateway receives the request. Published image limits are `max_images_per_request`, decoded `max_image_bytes` per image, and header-derived `max_image_pixels` per image. The aggregate base64 payload plus request envelope must still fit the 1 MiB HTTP body limit. Supported MIME types are `image/png`, `image/jpeg`, `image/webp`, `image/gif`, and `image/bmp`; decoded bytes must match the declared format before dispatch. Profiles without `visual` capability and nonzero image limits reject image parts before dispatch. Invalid image submissions fail closed with `image_input_unqualified`, `image_url_unqualified`, `unsupported_image_type`, `invalid_image_payload`, `image_format_mismatch`, `image_count_exceeded`, `image_bytes_exceeded`, or `image_pixels_exceeded` as applicable. An `Idempotency-Key`
