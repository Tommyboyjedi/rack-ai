...
Historical evidence is archived at `/srv/rack-ai-media/archive/qualification/pr24-restart-20260913/evidence`: deployment manifest/binary hashes; Rust, full-media and focused/final restart logs; `live-restart-proof.json`; workflow/history; Manager confirmation/Ready/image/Finish screenshots; final NVIDIA XML; and protected before/after/comparison JSON. The browser harness is `/srv/rack-ai-media/archive/qualification/pr24-restart-20260913/live-restart-proof.py`. Secrets and control state remain outside ComfyUI user data.
...
Bookmark [Rack launcher](https://gpurack.tailc214fc.ts.net/) on the authorized tailnet. Sign in using the existing operator credential. Start → Open ComfyUI; use Extensions → Restart → Confirm normally. The launcher shows Restarting and returns to Ready automatically. Finish remains the cleanup action.

The permanent deployment supersedes the original runtime paths. Use the current [qualification record](pr24-qualification.md) and [permanent rollback procedure](pr24-media-operations.md#validation-and-rollback). Historical unit/config copies are retained with the archived evidence; they must not be installed as current configuration. The qualified restart implementation is preserved unchanged by the permanent-path migration.
