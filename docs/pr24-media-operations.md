...
The model store remains `/srv/fast/comfyui-models-pr24` on SATA. The selected assets were copied from USB and independently hash-verified; the USB remains intact. The managed profile is Juggernaut Ragnarok. Krea2, LTX2.3 and CyberRealisticPony remain available to compatible native workflows without extending their qualification. GGUF is provided by the existing ComfyUI-GGUF node. No model download or paid inference is part of normal qualification.
