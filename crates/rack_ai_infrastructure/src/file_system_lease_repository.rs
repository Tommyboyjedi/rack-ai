...
impl LeaseRepository for FileSystemLeaseRepository {
    fn blocked_resources(&self, placement: &Placement) -> Result<Vec<String>, String> {
        self.authority().blocked(placement.resource_ids())
    }
    fn acquire(
        &self,
        request: &rack_ai_application::LeaseRequest<'_>,
    ) -> Result<rack_ai_application::LeaseHandle, String> {
        self.authority()
            .acquire(&crate::resource_reservations::ReservationRequest {
                owner: request.task_id.value().to_owned(),
                resources: request.placement.resource_ids().to_vec(),
                acquired_at: request.acquired_at.to_owned(),
                worker_ids: request.placement.worker_ids().to_vec(),
                model_ids: request.placement.model_ids().to_vec(),
            })
    }
    fn renew(&self, handle: &rack_ai_application::LeaseHandle) -> Result<(), String> {
        self.authority().verify(handle)
    }
    fn release(&self, handle: &rack_ai_application::LeaseHandle) -> Result<(), String> {
...
impl FileSystemLeaseRepository {
    fn authority(&self) -> crate::resource_reservations::ResourceReservations {
        crate::resource_reservations::ResourceReservations::new(self.paths.resource_root())
    }
}
...
mod tests {
    ...
    fn acquires_blocks_and_releases_resource_leases() {
        ...
        let lease_paths = repository
            .acquire(&rack_ai_application::LeaseRequest {
                task_id: &TaskId::new("task-lease".to_string()).unwrap(),
                placement: &placement,
                acquired_at: "2026-08-20T21:00:00Z",
            })
            .unwrap();
        ...
        assert!(lease_paths.paths.contains_key("gpu-2060"));
        ...
        repository.release(&lease_paths).unwrap();
    ...
    fn refuses_to_acquire_busy_resource() {
        ...
        repository
            .acquire(&rack_ai_application::LeaseRequest {
                task_id: &TaskId::new("first".to_string()).unwrap(),
                placement: &placement,
                acquired_at: "2026-08-20T21:00:00Z",
            })
            .unwrap();
        let error = repository
            .acquire(&rack_ai_application::LeaseRequest {
                task_id: &TaskId::new("second".to_string()).unwrap(),
                placement: &placement,
                acquired_at: "2026-08-20T21:01:00Z",
            })
            .unwrap_err();
