...
mod tests {
    #[test]
    fn profile_and_source_policy_are_scoped_and_exact() {
        let mut c: crate::config::Config = serde_json::from_str(include_str!(
            "../../../config/runtime/config.example.json"
        )).unwrap();
        crate::validation::validate(&c).unwrap();
        assert!(c.sources.iter().find(|s| s.source == "cb").unwrap().tags.contains(&"local-tts".into()));
        assert!(!c.sources.iter().find(|s| s.source == "athba").unwrap().tags.contains(&"local-tts".into()));
        c.devices.get_mut("gpu-2060").unwrap().uuid = "GPU-042e18f2-bf9f-c8f6-6975-6f25b15ac71d".into();
        assert_eq!(crate::validation::validate(&c).unwrap_err(), "chatterbox_exact_uuid_required");
        c.devices.get_mut("gpu-2060").unwrap().uuid = "GPU-357ef569-8fac-7c7d-ee1c-51677efb174f".into();
        let p = c.profiles.iter_mut().find(|p| p.tag == "local-tts").unwrap();
        p.protocols = vec![crate::protocol::Protocol::ChatCompletions];
        assert_eq!(crate::validation::validate(&c).unwrap_err(), "invalid_chatterbox_profile");
    }
}
