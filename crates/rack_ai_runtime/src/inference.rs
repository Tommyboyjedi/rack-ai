        ...
        self.service.authority.update(|s| {
            if let Some(work) = &request.work {
                if let Some(i) = crate::work::find(s, owner, &work.work_id) {
                    return if i.request.work.as_ref() == Some(work) { Ok(i.clone()) } else { Err("identity_conflict".into()) };
                }
                let selected = crate::reservation::select(s, (owner, &work.reservation_id, &work.service))?;
                if selected.id != request.reservation_id { return Err("service_not_reserved".into()); }
            }
            ...
            let workspace = request.work.as_ref().is_some_and(|w| w.workspace().is_some());
            ...
            if request.schema != VERSION || !valid_id(&request.submission_id) || request.timeout_seconds == 0
                || request.timeout_seconds > if workspace { crate::work_execution::MAX_WORKSPACE_SECONDS } else { d.profile.inference_seconds } || request.max_tokens == 0
            ...
            let invocation = Invocation { scope_access_hash: None, id: identity()?, owner: owner.into(), waiting_deadline: now() + wait, execution_deadline: None,
